...
def obtain_data():
    """Looks through all CSV files in the files folder"""
    with open("input_data.json") as f:
        #  perform calculation
        data = f.load()
        inputs = data.values()
        cqmSolver(inputs[0], inputs[1], inputs[2], inputs[3], inputs[4], inputs[5], inputs[6])
        f.close()

def cqmSolver(C, D, T, A, r_winter_columns, r_summer_columns, c_winter_columns, c_summer_columns):

...
def obtain_data():
    if os.path.exists("uploads/input_data.json"):
        with open("uploads/input_data.json", "r") as f:
            data = json.load(f)
            solveCQM(data.values[0], data.values[1], data.values[2], data.values[3], data.values[4], data.values[5], data.values[6])
            f.close()

def solveCQM(C, D, T, A, r_winter_columns, r_summer_columns, c_winter_columns, c_summer_columns):
    print("Reached CQM")
